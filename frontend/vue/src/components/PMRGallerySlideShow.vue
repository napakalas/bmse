<template>
  <transition name="modal">
    ...
    >
      ...
      <div class="vgs__nav  text-overline mb-4">
        <div>
          {{imageCaption}}
        </div>
        <div>
          <!-- <v-tooltip> -->
            <!-- <template #activator="{ on }"> -->
              <v-chip small 
                v-if="imageCellml" class="pa-4 ma-1">
                <a :href="imageCellml[0]" target="_blank">
                  <!-- {{ imageCellml[1].length > maxLength?'...'+imageCellml[1].substring(imageCellml[1].length-maxLength):imageCellml[1]}} -->
                  Open CellML
                  <v-icon small> mdi-open-in-new</v-icon>
                </a>
              </v-chip>
            <!-- </template> -->
            <!-- <span>Open CellML</span> -->
          <!-- </v-tooltip> -->
        <!-- </div> -->
        <!-- <div> -->
          <!-- <v-tooltip> -->
            <!-- <template #activator="{ on }"> -->
              <v-chip small 
                v-if="imageWorkspace" class="pa-4 ma-1">
                <a :href="imageWorkspace[0]" target="_blank">
                  <!-- {{ imageWorkspace[1].length > maxLength?'...'+imageWorkspace[1].substring(imageWorkspace[1].length-maxLength):imageWorkspace[1]}} -->
                  Open Workspace
                  <v-icon small> mdi-open-in-new</v-icon>
                </a>
              </v-chip>
        ...
        <!-- <div> -->
          <!-- <v-tooltip> -->
            <!-- <template #activator="{ on }"> -->
              <v-chip small 
                v-if="imageExposure" class="pa-4 ma-1">
                <a :href="imageExposure[0]" target="_blank">
                  <!-- {{ imageExposure[1].length > maxLength?'...'+imageExposure[1].substring(imageExposure[1].length-maxLength):imageExposure[1]}} -->
                  Open Exposure
                  <v-icon small> mdi-open-in-new</v-icon>
                </a>
              </v-chip>
...
export default {
  ...
  computed: {
    ...
    imageCellml() {
      const img = this.images[this.imgIndex];
      return [img.cellmlUrl, img.cellmlUrl.substring(img.cellmlUrl.indexOf('.org')+5)];
    },
    imageWorkspace() {
      const img = this.images[this.imgIndex];
      return [img.workspaceUrl, img.workspaceUrl.substring(img.workspaceUrl.indexOf('.org')+5)];
    },
    imageExposure() {
      const img = this.images[this.imgIndex];
      if (img.exposures.length > 0) {
        return [img.exposures[0], img.exposures[0].substring(img.exposures[0].indexOf('.org')+5)];
      }
      return null;
    },
    alt() {
